...
//		pref2ccmask(&cc_mask);
	...
	if ( rtsp_sess->content_base != NULL)
		if (*(rtsp_sess->pathname) != 0)
			...
			sprintf(b, "%s %s %s"RTSP_EL"CSeq: %d"RTSP_EL, PLAY_TKN, rtsp_sess->content_base, RTSP_VER, ++(rtsp_sess->CSeq));
	else
		sprintf(b, "%s %s %s"RTSP_EL"CSeq: %d"RTSP_EL, PLAY_TKN, rtsp_sess->pathname, RTSP_VER, ++(rtsp_sess->CSeq));

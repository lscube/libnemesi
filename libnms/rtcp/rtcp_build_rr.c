/* * 
 *  $Id:rtcp_build_rr.c 267 2006-01-12 17:19:45Z shawill $
 *  
...
int rtcp_build_rr(rtp_session * rtp_sess, rtcp_pkt * pkt)
{
	...
	for (stm_src = rtp_sess->ssrc_queue; stm_src; stm_src = stm_src->next) {
		...
		if (stm_src->ssrc_stats.received_prior != stm_src->ssrc_stats.received) {
			...
			expected =
			    stm_src->ssrc_stats.cycles + stm_src->ssrc_stats.max_seq - stm_src->ssrc_stats.base_seq + 1;
			...
			rr->last_sr =
			    htonl(((stm_src->ssrc_stats.ntplastsr[0] & 0x0000ffff) << 16) | ((stm_src->ssrc_stats.

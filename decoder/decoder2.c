...
void *decoder(void *args)
{
	...
	do {
		...
		for (stm_src = rtp_active_ssrc_queue(rtsp_get_rtp_queue(rtsp_ctl)); stm_src;
			...
			if (timeval_subtract(NULL, &tv_elapsed, &tvstart)) {
				if (!rtp_fill_buffer(stm_src, &fr, &config) && DECODERS(stm_src->rtp_sess->park)[fr.pt]) {
					...
					if (config.len > 0)	// || if (config.data)
						DECODERS(stm_src->rtp_sess->park)[fr.pt] (config.data, config.len,
											  nms_outc);
					...
					nms_statusprintf(BUFFERS_STATUS,
							 ...
							 (((float) ((stm_src->rtp_sess->bp).flcount) /
							   (float) BP_SLOT_NUM) * 100.0), audio_sysbuff * 100.0,
							 video_sysbuff * 100.0);
				}

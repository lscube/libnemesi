...
void *decoder(void *args)
{
	...
	for (rtp_sess = rtsp_get_rtp_queue(rtsp_ctl); rtp_sess;
		...
		if (!(rtp_sess->park = malloc(sizeof(decoders)))) {
			nms_printf(NMSML_FATAL,
				   "Could not allocate memory for decoders\n");
			return NULL;
		}
		...
		for (fmt = rtp_sess->announced_fmts; fmt; fmt = fmt->next) {
			if (RTPPT_ISDYNAMIC(fmt->pt)) {
				nms_printf(NMSML_WARN,
					   "dynamic payload type encountered: %u - %s\n",
	...
	do {
		...
		for (stm_src =
		     rtp_active_ssrc_queue(rtsp_get_rtp_queue(rtsp_ctl));
			...
			if (timeval_subtract(NULL, &tv_elapsed, &tvstart)) {
				if (!rtp_fill_buffer(stm_src, &fr, &config)
				    && DECODERS(stm_src->rtp_sess->park)[fr.
									 pt]) {
					nms_outc->elapsed = ts_next * 1000;
					if (config.len > 0)	// || if (config.data)
						DECODERS(stm_src->rtp_sess->
							 park)[fr.pt] (config.
								       data,
								       config.
								       len,
					...
					DECODERS(stm_src->rtp_sess->park)[fr.
									  pt]
					    (fr.data, fr.len, nms_outc);
					if (nms_outc->audio)
						nms_outc->audio->functions->
						    control(ACTRL_GET_SYSBUF,
							    &audio_sysbuff);
					if (nms_outc->video)
						nms_outc->video->functions->
						    control(VCTRL_GET_SYSBUF,
							    &video_sysbuff);
		...
		if (ts_min_next) {	// is there next packet?
			...
			if (!timeval_subtract(&tvsleep, &tv_min_next, &tvstop)
			    && (tvsleep.tv_usec > 10000)) {

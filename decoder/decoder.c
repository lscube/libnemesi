...
#else
			for (stm_src = rtp_active_ssrc_queue(rtp_sess_head); stm_src;
				...
				if ((pkt = rtp_get_pkt(stm_src, &len))) {
					...
					nms_printf(NMSML_DBG3, "RTP Timestamp:%lu\n", ntohl(pkt->time));
					 /**/
					    ts_elapsed =
					    ((double) (ntohl(pkt->time) - stm_src->ssrc_stats.firstts)) /
					    (double) rtp_sess->ptdefs[pkt->pt]->rate;
...
//                                              len= (stm_src->po.pobuff[stm_src->po.potail]).pktlen;
						if (len) {
							len -=
							    ((uint8 *) (pkt->data) - (uint8 *) pkt) - pkt->cc -
						...
						if ((len != 0) && (!strcmp(output_pref, "disk"))) {
							if (nms_outc->diskwriter)
								diskwriter(nms_outc->diskwriter, pkt->pt,
									   ((char *) pkt->data + pkt->cc + SKIP),
									   len - SKIP);
							...
							if (!strcmp(output_pref, "card")) {
								...
								decoders[pkt->pt] (((char *) pkt->data + pkt->cc), len,
										   nms_outc);
								if (nms_outc->audio)
									nms_outc->audio->functions->
									    control(ACTRL_GET_SYSBUF, &audio_sysbuff);
								if (nms_outc->video)
									nms_outc->video->functions->
									    control(VCTRL_GET_SYSBUF, &video_sysbuff);

								// AUDIO
								if (buffering_audio) {
									if (audio_sysbuff >
									    /*0.1 */ AUDIO_SYS_BUFF /*0.99 */ ) {
										buffering_audio = 0;
										// start playing audio
...
*/
						 /**/
						    nms_statusprintf(BUFFERS_STATUS,
								     ...
								     (((float) ((rtp_sess->bp).flcount) /
								       (float) BP_SLOT_NUM) * 100.0),
								     audio_sysbuff * 100.0, video_sysbuff * 100.0);

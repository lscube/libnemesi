		...
		do {
			...
			for (stm_src = rtp_active_ssrc_queue(rtp_sess_head);
				...
				if ((pkt = rtp_get_pkt(stm_src, &len))) {
					...
					timeval_add(&tv_elapsed, &tv_elapsed,
						    &startime);
...
*/
		...
		if ((body_usec =
		     tvbody.tv_sec * 1000000 + tvbody.tv_usec) >
		    (GRAIN * 1000 - offset_usec)) {
			cycles = (body_usec + offset_usec) / (GRAIN * 1000);
			offset_usec =
			    (body_usec + offset_usec) % (GRAIN * 1000);
		} else {

			tvdiff.tv_sec = tvsleep.tv_sec = tvbody.tv_sec;
			tvdiff.tv_usec = tvsleep.tv_usec =
			    GRAIN * 1000 - tvbody.tv_usec - offset_usec;
			if (tvdiff.tv_usec < 0) {
			...
			if ((select_usec =
			     tvsel.tv_sec * 1000000 + tvsel.tv_usec) >
			    diff_usec) {
				cycles =
				    (select_usec - diff_usec) / (GRAIN * 1000);
			}
			offset_usec =
			    (select_usec - diff_usec) % (GRAIN * 1000);
		}
		if (!strcmp(get_pref("output"), "card")) {
			...
			nms_outc->audio->functions->control(ACTRL_GET_SYSBUF,
							    &audio_sysbuff);

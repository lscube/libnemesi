...
nms_audio_buffer *ab_init(uint32 buff_size)
{
	...
	if ((buff =
	     (struct audio_buff *) malloc(sizeof(struct audio_buff))) == NULL) {
		nms_printf(NMSML_FATAL, "Cannot allocate memory.\n");
		return NULL;
	}

	if (((buff->audio_data) =
	     (uint8 *) malloc(buff_size * sizeof(uint8))) == NULL)
	...
	if (pthread_mutexattr_setpshared(&mutex_attr, PTHREAD_PROCESS_SHARED) >
	    0)
		RET_ERR(NMSML_FATAL,
			"Could not set mutex attribute (PROCESS_SHARED)!\n");
	return NULL;
#endif
#endif
	...
	if (pthread_condattr_init(&cond_attr) > 0)
		RET_ERR(NMSML_FATAL,
			"Could not init condition variable attributes!\n");

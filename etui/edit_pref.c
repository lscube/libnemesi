/* * 
 *  ./etui/edit_pref.c: $Revision: 1.1 $ -- $Date: 2003/01/13 16:49:48 $
 *  
...
int edit_pref(struct RTSP_args *rtsp_args, char *argstr)
{
	char *name, *value;
	int i=0;
	
	*(argstr+strlen(argstr)-1)='\0';
	argstr++; /* c'è lo spazio all'inizio, lo togliamo */
	...
	if ( strstr(preferences[i].available, value) == NULL) {
